...
def initiateTransactionRequest(bank, account, challenge_type, cp_bank, cp_account):
    ...
    challenge_type + '"}'
    response = requests.post(u"{0}/obp/v4.0.0/banks/{1}/accounts/{2}/owner/transaction-request-types/{3}/transaction-requests".format(BASE_URL, bank, account, challenge_type), data=payload, headers=mergeHeaders(DL_TOKEN, CONTENT_JSON))
    return response.json()
